...
import { skillPostApi, showToast, showLoading, hideLoading } from '../../utils/api'
import { 
  SKILL_CATEGORIES, 
  SKILL_ROLES, 
  EXPERIENCE_LEVELS, 
...
import type { CreateSkillPostDTO, SkillCategory, SkillRole, ExperienceLevel, PriceType, ContactMethod } from '@coshub/types'
import './create.scss'
...
export default class CreateSkillPost extends Component<{}, State> {
  ...
  // 处理嵌套字段变化
  handleNestedInputChange = (parentField: string, field: string, value: any) => {
    this.setState({
      formData: {
        ...this.state.formData,
        [parentField]: {
  ...
  render() {
    const { formData, categoryIndex, roleIndex, experienceIndex, priceTypeIndex, contactMethodIndex, cityIndex } = this.state
        ...
        <View className='form-section'>
          ...
          <View className='form-row'>
            <View className='form-item half'>
              ...
              >
                <View className='picker-view'>
                  <Text>{cityIndex === 0 ? '选择城市' : cityOptions[cityIndex]}</Text>
                </View>
              </Picker>
              {cityIndex === 0 && (
                <Input
          ...
          <View className='form-item'>
            ...
            />
            <Text className='char-count'>{formData.description.length}/2000</Text>
          </View>
          ...
          <View className='form-item'>
            ...
            />
            <Text className='char-count'>已添加 {formData.images.length} 张图片</Text>
          </View>
          ...
          {formData.price.type === 'fixed' && (
            <View className='form-item'>
              ...
              <Input
                ...
                onInput={(e) => this.handleNestedInputChange('price', 'amount', Number(e.detail.value))}
              />
            </View>
          )}
        </View>
        ...
        <View className='form-section'>
          ...
          <View className='form-item'>
            ...
            <Input
              ...
              onInput={(e) => this.handleNestedInputChange('contactInfo', 'wechat', e.detail.value)}
            />
          </View>
          ...
          <View className='form-item'>
            ...
            <Input
              ...
              onInput={(e) => this.handleNestedInputChange('contactInfo', 'qq', e.detail.value)}
            />
          </View>
